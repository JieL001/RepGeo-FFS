import torch,pdb,logging,timm,math
...
from core.submodule import (
    ...
    ChannelAttentionEnhancement, disparity_regression, disparity_regression_local, context_upsample,
    ...
    build_gwc_volume_local, build_concat_volume_local,
)
...
class FastFoundationStereo(nn.Module):
  ...
  def compute_coarse_center(self, feat_left, feat_right, coarse_maxdisp, upsample_scale):
    coarse_groups = min(self.cv_group, feat_left.shape[1])
    while feat_left.shape[1] % coarse_groups != 0 and coarse_groups > 1:
      coarse_groups -= 1
    coarse_volume = build_gwc_volume_optimized_pytorch1(
      feat_left,
      feat_right,
      coarse_maxdisp,
      coarse_groups,
    ...
    coarse_logits = coarse_volume.mean(dim=1)
    coarse_prob = F.softmax(coarse_logits, dim=1)
    coarse_disp = disparity_regression(coarse_prob, coarse_maxdisp)
    center_disp = F.interpolate(coarse_disp, scale_factor=upsample_scale, mode='bilinear', align_corners=True) * float(upsample_scale)
    center_disp = center_disp.clamp(0, self.args.max_disp // 4 - 1)
    return center_disp
  ...
  def forward(self, image1, image2, iters=12, test_mode=False, low_memory=False, init_disp=None, profile=False, optimize_build_volume='pytorch1', return_aux=False):
    """ Estimate disparity between pair of frames """
    B,C,H,W = image1.shape
    low_memory = low_memory or (self.args.get('low_memory', False))
    adaptive_refine = bool(self.args.get('adaptive_refine', False))
    adaptive_warmup_iters = int(self.args.get('adaptive_refine_warmup', 1))
    adaptive_threshold = float(self.args.get('adaptive_refine_threshold', 0.5))
    adaptive_hard = bool(self.args.get('adaptive_refine_hard', True))
    narrow_band = bool(self.args.get('narrow_band', False))
    band_radius = int(self.args.get('band_radius', 8))
    ...
    with torch.amp.autocast('cuda', enabled=self.args.mixed_precision, dtype=U.AMP_DTYPE):
      ...
      if narrow_band:
        local_count = 2 * band_radius + 1
        padded_count = max(8, math.ceil(local_count / 8) * 8)
        pad_left = (padded_count - local_count) // 2
        pad_right = padded_count - local_count - pad_left
        with torch.no_grad():
          if optimize_build_volume=='pytorch1':
            coarse_gwc = build_gwc_volume_optimized_pytorch1(features_left[0], features_right[0], self.args.max_disp//4, self.cv_group, normalize=self.args.normalize)
          elif optimize_build_volume=='triton':
            coarse_gwc = build_gwc_volume_triton(features_left[0], features_right[0], self.args.max_disp//4, self.cv_group, normalize=self.args.normalize)
          else:
            raise RuntimeError(f"Invalid optimize_build_volume: {optimize_build_volume}")
          coarse_concat = build_concat_volume_optimized_pytorch1(left_tmp, right_tmp, maxdisp=self.args.max_disp//4)
          coarse_volume = torch.cat([coarse_gwc, coarse_concat], dim=1)
          del coarse_gwc, coarse_concat
          coarse_volume = self.corr_stem(coarse_volume)
          coarse_volume = self.corr_feature_att(coarse_volume, features_left[0])
          coarse_volume = self.cost_agg(coarse_volume, features_left)
          coarse_logits = self.classifier(coarse_volume).squeeze(1)
          coarse_prob = F.softmax(coarse_logits, dim=1)
          coarse_center = disparity_regression(coarse_prob, self.args.max_disp//4).detach()
          del coarse_volume, coarse_logits, coarse_prob
        local_offsets = torch.arange(-band_radius, band_radius + 1, device=features_left[0].device, dtype=features_left[0].dtype)
        gwc_volume = build_gwc_volume_local(
          features_left[0],
          features_right[0],
          coarse_center,
          radius=band_radius,
          ...
          normalize=self.args.normalize,
          maxdisp=self.args.max_disp // 4,
        )
        concat_volume = build_concat_volume_local(
          left_tmp,
          right_tmp,
          coarse_center,
          radius=band_radius,
        ...
        if pad_left or pad_right:
          gwc_volume = F.pad(gwc_volume, (0, 0, 0, 0, pad_left, pad_right))
          concat_volume = F.pad(concat_volume, (0, 0, 0, 0, pad_left, pad_right))
          local_offsets = F.pad(local_offsets, (pad_left, pad_right))
        local_valid_mask = torch.zeros((1, padded_count, 1, 1), device=features_left[0].device, dtype=torch.bool)
        local_valid_mask[:, pad_left:pad_left + local_count] = True
      else:
        if optimize_build_volume=='pytorch1':
      ...
      if narrow_band:
        logits = logits.masked_fill(~local_valid_mask, torch.finfo(logits.dtype).min)
      prob = F.softmax(logits, dim=1)
      if init_disp is None:
        if narrow_band:
          init_disp = disparity_regression_local(prob, coarse_center, local_offsets)
        else:
    ...
    disp_preds = []
    aux_outputs = {'refine_gates': []}
    if narrow_band and init_disp is not None and return_aux:
      aux_outputs['coarse_center'] = coarse_center
    ...
    for itr in range(iters):
      ...
      with torch.amp.autocast('cuda', enabled=self.args.mixed_precision, dtype=U.AMP_DTYPE):
        net_list, mask_feat_4, delta_disp, refine_gate = self.update_block(net_list, inp_list, geo_feat.to(self.dtype), disp, att)

      if return_aux:
        aux_outputs['refine_gates'].append(refine_gate)

      if adaptive_refine and itr >= adaptive_warmup_iters:
        if test_mode and adaptive_hard:
          refine_gate = (refine_gate > adaptive_threshold).to(self.dtype)
        else:
          refine_gate = refine_gate.to(self.dtype)
        delta_disp = delta_disp * refine_gate
    ...
    if test_mode:
      if return_aux:
        return disp_up, aux_outputs
      return disp_up

    if return_aux:
      return init_disp, disp_preds, aux_outputs
...
class TrtPostRunner(nn.Module):
  ...
  def forward(self, features_left_04, features_left_08, features_left_16, features_left_32, features_right_04, stem_2x, gwc_volume):
    ...
    for itr in range(self.args.valid_iters):
      ...
      net_list, mask_feat_4, delta_disp, _ = self.update_block(net_list, inp_list, geo_feat.to(self.dtype), disp, att)
