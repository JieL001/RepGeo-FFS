...
class BasicSelectiveMultiUpdateBlock(nn.Module):
    def __init__(self, args, hidden_dim=128, volume_dim=8):
        ...
        self._ensure_refine_gate()

    def _ensure_refine_gate(self):
        if hasattr(self, 'refine_gate') and self.refine_gate is not None:
            return
        inferred_hidden = getattr(self, 'hidden_dim', None)
        if inferred_hidden is None:
            inferred_hidden = self.disp_head.conv[0].in_channels
            self.hidden_dim = inferred_hidden
        gate_hidden = max(inferred_hidden // 2, 32)
        self.refine_gate = nn.Sequential(
            nn.Conv2d(inferred_hidden, gate_hidden, 3, padding=1),
            nn.ReLU(inplace=True),
            nn.Conv2d(gate_hidden, 1, 3, padding=1),
        )
        ref_param = next(self.disp_head.parameters(), None)
        if ref_param is not None:
            self.refine_gate = self.refine_gate.to(device=ref_param.device, dtype=ref_param.dtype)
    ...
    def forward(self, net, inp, corr, disp, att):
        ...
        refine_gate = torch.sigmoid(self.refine_gate(net[0]))

        mask = .25 * self.mask(net[0])
        return net, mask, delta_disp, refine_gate
