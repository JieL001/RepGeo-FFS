...
def sample_features_by_disparity(targetimg_fea: torch.Tensor, disp: torch.Tensor):
    """
    @targetimg_fea: (B,C,H,W)
    @disp: (B,1,H,W) in disparity units at current feature scale
    """
    B, C, H, W = targetimg_fea.shape
    xs = torch.arange(W, device=targetimg_fea.device, dtype=disp.dtype).view(1, 1, W).expand(B, H, W)
    ys = torch.arange(H, device=targetimg_fea.device, dtype=disp.dtype).view(1, H, 1).expand(B, H, W)
    sample_x = xs - disp.squeeze(1)
    sample_x = 2.0 * sample_x / max(W - 1, 1) - 1.0
    sample_y = 2.0 * ys / max(H - 1, 1) - 1.0
    grid = torch.stack((sample_x, sample_y), dim=-1)
    sampled = F.grid_sample(targetimg_fea, grid, mode='bilinear', padding_mode='zeros', align_corners=True)
    return sampled


def build_gwc_volume_local(refimg_fea: torch.Tensor, targetimg_fea: torch.Tensor, center_disp: torch.Tensor, radius: int, num_groups: int, normalize=True, maxdisp=None):
    dtype = refimg_fea.dtype
    B, C, H, W = refimg_fea.shape
    channels_per_group = C // num_groups
    if maxdisp is None:
    ...
    ref_groups = refimg_fea.view(B, num_groups, channels_per_group, H, W)
    if normalize:
        ref_groups = F.normalize(ref_groups.float(), dim=2).to(dtype)

    volume = []
    for offset in range(-radius, radius + 1):
        disp = (center_disp + float(offset)).clamp(0, maxdisp - 1)
        sampled = sample_features_by_disparity(targetimg_fea, disp)
        sampled_groups = sampled.view(B, num_groups, channels_per_group, H, W)
        if normalize:
            sampled_groups = F.normalize(sampled_groups.float(), dim=2).to(dtype)
        corr = (ref_groups * sampled_groups).sum(dim=2)
        volume.append(corr)
    return torch.stack(volume, dim=2).contiguous()


def build_concat_volume_local(refimg_fea: torch.Tensor, targetimg_fea: torch.Tensor, center_disp: torch.Tensor, radius: int, maxdisp=None):
    B, C, H, W = refimg_fea.shape
    if maxdisp is None:
        maxdisp = W
    ...
    for offset in range(-radius, radius + 1):
        disp = (center_disp + float(offset)).clamp(0, maxdisp - 1)
        sampled = sample_features_by_disparity(targetimg_fea, disp)
        volume.append(torch.cat((refimg_fea, sampled), dim=1))
    return torch.stack(volume, dim=2).contiguous()


def disparity_regression_local(x: torch.Tensor, center_disp: torch.Tensor, offsets: torch.Tensor):
    assert len(x.shape) == 4
    offsets = offsets.to(dtype=x.dtype, device=x.device).reshape(1, -1, 1, 1)
    residual = torch.sum(x * offsets, dim=1, keepdim=True)
    return center_disp + residual
